...
\begin{document}
	\begin{titlepage}
		\begin{center}
			...
			\begin{figure}
				\centering
				\includegraphics[width=0.3\linewidth]{"matcom_logo"}
				\label{fig:matcomlogo}
			\end{figure}
			
			\vspace{2.5cm}
			\Huge
			\colorbox{yellow!40}{\textbf{\textit{Proyecto de Simulación }}}\\
			\vspace{1cm}
			
			\Large
			\textbf{Simulando la Evolución de un Poblado}\\
...
\begin{center}\rule{0.7\linewidth}{0.4pt}\end{center}

\section{Ambigüedades del problema}
...
\begin{itemize}
    ...
    \item \textbf{Acumulada sobre todo el rango}: hay 25\% de probabilidad de que, durante los 12 años de la infancia, el niño muera en algún momento. Esto da una mortalidad infantil del 25\% en total, dura pero  compatible con un modelo demográfico funcional.
\end{itemize}
...
La primera es más natural. Decir que alguien ``desea pareja'' sugiere algo del orden de los rasgos o la situación vital.
...
¿Una persona con exactamente 12 años cae en el rango 0--12 o en el rango 12--45? El enunciado no aclara, pero las tablas se solapan en los extremos (45 aparece como límite superior de 12--45 y como límite inferior de 45--76). La convención usual es semi-abierta: $[\mathrm{min}, \mathrm{max})$.
...
\subsection{Decisión principal: probabilidades como acumuladas por rango}
...
$[\mathrm{min}, \mathrm{max})$. Una persona con exactamente $12.0$ años cae en el rango 12--45 (no en 0--12). Esto es consistente con la práctica habitual en estadística y elimina cualquier ambigüedad en los bordes.

\subsection{Duelo exponencial con $\lambda$ como media}

Se usó \texttt{random.expovariate(1/$\lambda$)} donde $\lambda$ es la media en meses, y se redondeó a entero con mínimo 1. Eso da una distribución exponencial con media exactamente $\lambda$ y varianza $\lambda^2$, lo que produce duelos típicamente cortos pero con cola larga: posible que una persona quede sola durante años después de una ruptura severa.
...
Por cada réplica se forman $\sim$110 parejas y se rompen $\sim$102. Es decir, casi todas las parejas que se forman se disuelven antes de los 100 años. Pero el dato más interesante es la \textbf{tasa de embarazos por pareja}: 35 nacimientos en 110 parejas $\approx 0.32$ nacimientos por pareja.

Las parejas son mayoritariamente estériles porque no duran lo suficiente o porque se forman entre personas fuera del rango fértil. Si las parejas duraran el doble (probabilidad de ruptura anual = 0.10 en lugar de 0.20), la duración media saltaría a $\sim$9 años, y la fertilidad por pareja crecería desproporcionadamente --- quizá $\times 2$ o $\times 3$. \textbf{La duración de la pareja es el parámetro de mayor apalancamiento en este modelo}.
